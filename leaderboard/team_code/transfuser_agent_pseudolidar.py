...
class TransFuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        # Pseudolidar setup
        self.pcl_generator = PointCloudGenerator(fov=100, image_dims=(300, 400))
        with open('pseudolidar/extrinsics.json', mode='r') as f:
            camera_poses = json.load(f)

        front_pose = PseudoLidarGenerator.load_pose(camera_poses['cam_front'])
        left_pose = PseudoLidarGenerator.load_pose(camera_poses['cam_left'])
        right_pose = PseudoLidarGenerator.load_pose(camera_poses['cam_right'])
        rear_pose = PseudoLidarGenerator.load_pose(camera_poses['cam_rear'])
        self.lidar_generator = PseudoLidarGenerator([front_pose, left_pose, right_pose, rear_pose])

        self.input_buffer = {'rgb': deque(), 'rgb_left': deque(), 'rgb_right': deque(), 'rgb_rear': deque(),
                             'depth_front': deque(), 'depth_left': deque(), 'depth_right': deque(), 'depth_rear': deque(),
                             'lidar': deque(), 'pseudolidar': deque(), 'gps': deque(), 'thetas': deque()}
    ...
    def tick(self, input_data):
        ...
        depth_front = input_data['depth_front'][1][:, :, [2, 1, 0]]
        depth_left = input_data['depth_front'][1][:, :, [2, 1, 0]]
        depth_right = input_data['depth_right'][1][:, :, [2, 1, 0]]
        depth_rear = input_data['depth_rear'][1][:, :, [2, 1, 0]]

        depth_front = PointCloudGenerator.process_depth(depth_front)
        depth_left = PointCloudGenerator.process_depth(depth_left)
        depth_right = PointCloudGenerator.process_depth(depth_right)
        depth_rear = PointCloudGenerator.process_depth(depth_rear)

        # Put pseudolidar processing here
        front_pc = self.pcl_generator.generate(depth_front, max_depth=0.03)
        left_pc = self.pcl_generator.generate(depth_left, max_depth=0.03)
        right_pc = self.pcl_generator.generate(depth_right, max_depth=0.03)
        rear_pc = self.pcl_generator.generate(depth_rear, max_depth=0.03)
        pseudolidar = self.lidar_generator.generate([front_pc, left_pc, right_pc, rear_pc])
        
        mesh = PseudoLidarGenerator.reconstruct_mesh(pseudolidar)
        points = PseudoLidarGenerator.raycast_mesh(mesh, n_measurements=400)
        points = points[(points[:, 0] <= 10.) & (points[:, 1] <= 10.)]
        pseudolidar = PseudoLidarGenerator.post_process(points)
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.step < self.config.seq_len:
            ...
            self.input_buffer['pseudolidar'].append(tick_data['pseudolidar'])
        ...
        # self.input_buffer['lidar'].append(tick_data['lidar'])
        self.input_buffer['pseudolidar'].popleft()
        self.input_buffer['pseudolidar'].append(tick_data['pseudolidar'])
        ...
        if(self.step % 2 == 0 or self.step <= 4):
            for i, lidar_hist in enumerate(self.input_buffer['pseudolidar']):
                """
                ...
                """
                lidar_hist = torch.from_numpy(lidar_hist.copy())
                lidar_hist = lidar_hist.unsqueeze(0)
                self.lidar_processed = list()
                self.lidar_processed.append(
                    lidar_hist.to('cuda', dtype=torch.float32))
