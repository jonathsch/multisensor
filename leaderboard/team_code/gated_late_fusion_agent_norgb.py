...
class LateFusionAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.net = LateFusion(self.config, 'cuda', use_robust_gate=True)
    ...
    def tick(self, input_data):
        ...
        rgb_left = np.zeros_like(rgb_left)
        rgb_right = np.zeros_like(rgb_right)
        rgb_rear = np.zeros_like(rgb_rear)
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.step < self.config.seq_len:
            ...
            if not self.config.ignore_sides:
                rgb_left = torch.from_numpy(scale_and_crop_image(Image.fromarray(
                    tick_data['rgb_left']), scale=self.config.scale, crop=self.config.input_resolution)).unsqueeze(0)
                ...
                rgb_right = torch.from_numpy(scale_and_crop_image(Image.fromarray(
                    tick_data['rgb_right']), scale=self.config.scale, crop=self.config.input_resolution)).unsqueeze(0)
            ...
            if not self.config.ignore_rear:
                rgb_rear = torch.from_numpy(scale_and_crop_image(Image.fromarray(
                    tick_data['rgb_rear']), scale=self.config.scale, crop=self.config.input_resolution)).unsqueeze(0)
        ...
        if not self.config.ignore_sides:
            ...
            self.input_buffer['rgb_left'].append(
                rgb_left.to('cuda', dtype=torch.float32))
            encoding.append(self.net.image_encoder(
                list(self.input_buffer['rgb_left'])))
            ...
            self.input_buffer['rgb_right'].append(
                rgb_right.to('cuda', dtype=torch.float32))
            encoding.append(self.net.image_encoder(
                list(self.input_buffer['rgb_right'])))
        ...
        if not self.config.ignore_rear:
            ...
            self.input_buffer['rgb_rear'].append(
                rgb_rear.to('cuda', dtype=torch.float32))
            encoding.append(self.net.image_encoder(
                list(self.input_buffer['rgb_rear'])))
