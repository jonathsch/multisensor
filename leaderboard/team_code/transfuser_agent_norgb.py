...
class TransFuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.input_buffer = {'rgb': deque(), 'rgb_left': deque(), 'rgb_right': deque(), 'rgb_rear': deque(),
                             ...
                             'lidar': deque(), 'gps': deque(), 'thetas': deque()}
    ...
    def tick(self, input_data):
        ...
        rgb = cv2.cvtColor(np.zeros_like(input_data['rgb'][1][:, :, :3]), cv2.COLOR_BGR2RGB)
        rgb_left = cv2.cvtColor(
            np.zeros_like(input_data['rgb_left'][1][:, :, :3]), cv2.COLOR_BGR2RGB)
        rgb_right = cv2.cvtColor(
            np.zeros_like(input_data['rgb_right'][1][:, :, :3]), cv2.COLOR_BGR2RGB)
        rgb_rear = cv2.cvtColor(
            np.zeros_like(input_data['rgb_rear'][1][:, :, :3]), cv2.COLOR_BGR2RGB)
        ...
        front_pc = self.pcl_generator.generate(depth_front)
        left_pc = self.pcl_generator.generate(depth_left)
        right_pc = self.pcl_generator.generate(depth_right)
        rear_pc = self.pcl_generator.generate(depth_rear)

        pseudolidar = self.lidar_generator.generate([front_pc, left_pc, right_pc, rear_pc])
        pseudolidar = PseudoLidarGenerator.post_process(pseudolidar)
        pseudolidar = PseudoLidarGenerator.project_to_bev(pseudolidar)
        """
