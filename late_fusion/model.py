...
class LateFusion(nn.Module):
    ...
    def __init__(self, config, device, use_robust_gate=False):
        super().__init__()
        self.config = config
        self.device = device
        self.use_robust_gate = use_robust_gate
        ...
        self.lidar_encoder = LidarEncoder(
            num_classes=512, in_channels=2).to(self.device)
        ...
        self.join_robust = nn.Sequential(
            nn.Linear(1024, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 2),
        ).to(self.device)

        self.final_layers_robust = nn.Sequential(
            nn.Linear(512, 64),
            # nn.ReLU(inplace=True),
            # nn.Linear(256, 128),
            # nn.ReLU(inplace=True),
    ...
    def forward(self, feature_emb, target_point):
        ...
        '''
        if not self.use_robust_gate:
            feature_emb = sum(feature_emb) 
            z = self.join(feature_emb)
        else:
            z = self.join_robust(torch.cat(feature_emb,dim=1))
            embeddings_weights = torch.split(z, 1, 1)
            feature_emb = sum(
                [torch.mul(w,f_emb) for w, f_emb in zip(embeddings_weights, feature_emb)])
            z = self.final_layers_robust(feature_emb)
