...
parser.add_argument('--id', type=str, default='late_fusion',
...
parser.add_argument('--sensor-failures', '-sf', nargs='+', type=str, choices=[
                    'rgb', 'lidar'], default=None, help='If sensor is added here, it will be randomly dropped during some epochs during training to make it more robust against sensor failure')
parser.add_argument('--prob', '-p', type=float, default=0.3,
                    help='probability with which to drop a sensor')
parser.add_argument('--use-robust-gate', '-robust', action='store_true')
    ...
    def train(self):
        ...
        for data in tqdm(dataloader_train):
            ...
            for i in range(config.seq_len):
                self.current_sensor_failure_used = random.choice(
                    self.sensor_failures)
                if self.current_sensor_failure_used == 'rgb':
                    possible_fronts = [fronts_in[i],
                                       torch.zeros_like(fronts_in[i])]
                    front = random.choices(possible_fronts, weights=[
                                           1-args.prob, args.prob])[0]
                ...
                if not config.ignore_rear:
                    rears.append(rears_in[i].to(
                        args.device, dtype=torch.float32))
                if self.current_sensor_failure_used == 'lidar':
                    possible_lidars = [lidars_in[i],
                                       torch.zeros_like(lidars_in[i])]
                    # lidar = np.random.choice(possible_lidars, 1, [1-args.prob, args.prob]).tolist()[0]
                    lidar = random.choices(possible_lidars, weights=[
                                           1-args.prob, args.prob])[0]
...
# Model
model = LateFusion(config, args.device, use_robust_gate=args.use_robust_gate)
