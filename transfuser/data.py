...
class CARLA_Data(Dataset):
    ...
    def __init__(self, root, config):
        ...
        self.use_pseudolidar = config.use_pseudolidar
        ...
        for sub_root in tqdm(root, file=sys.stdout):
            if self.use_pseudolidar:
                preload_file = os.path.join(sub_root, 'rg_pseudolidar_diag_pl_'+str(self.seq_len)+'_'+str(self.pred_len)+'.npy')
            else:
                ...
                for route in routes:
                    ...
                    for seq in range(num_seq):
                        ...
                        for i in range(self.seq_len):
                            ...
                            # point cloud
                            if self.use_pseudolidar:
                                lidars.append(route_dir + f"/pseudolidar/{str(seq*self.seq_len+1+i).zfill(4)}.npy")
                            else:
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len + self.pred_len):
            ...
            if i < self.seq_len:
                ...
                if self.use_pseudolidar:
                    full_lidar[i] = full_lidar[i][:, [1, 0, 2]]
