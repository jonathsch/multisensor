...
class Encoder(nn.Module):
    ...
    def forward(self, image_list, lidar_list, velocity):
        ...
        if self.classification:
            fused_features = torch.cat([image_features, lidar_features], dim=-1).squeeze()
        else:
...
class TransFuser(nn.Module):
    ...
    def __init__(self, config, device, use_classification_branch=False):
        super().__init__()
        self.device = device
        self.config = config
        self.pred_len = config.pred_len
        self.use_classification_branch = use_classification_branch

        self.turn_controller = PIDController(K_P=config.turn_KP, K_I=config.turn_KI, K_D=config.turn_KD, n=config.turn_n)
        self.speed_controller = PIDController(K_P=config.speed_KP, K_I=config.speed_KI, K_D=config.speed_KD, n=config.speed_n)

        self.encoder = Encoder(config, classification=self.use_classification_branch).to(self.device)
        ...
        if self.use_classification_branch:
            ...
            self.classification_join = nn.Sequential(
                                nn.Linear(1027, 256),
                                nn.ReLU(inplace=True),
                                nn.Linear(256, 128),
                                nn.ReLU(inplace=True),
    ...
    def forward(self, image_list, lidar_list, target_point, velocity):
        ...
        if self.use_classification_branch:
            logits = self.classification_branch(fused_features).detach()
            fused_features = torch.cat([logits, fused_features], dim=-1)
            z = self.classification_join(fused_features)
            if len(z.shape) == 1:
                z = z.unsqueeze(0)
        else:
